...
export class MockLLMProvider implements LLMProvider {
  ...
  async complete<T = string>(req: LLMRequest): Promise<LLMResponse<T>> {
    ...
    // Simulate realistic per-call latency (400â€“1200ms).
...
// prototype â€” kept here to make the swap obvious.
...
function mockCritique(userPrompt: string): Critique {
  ...
  if (firstLine.length > 140) issues.push('Hook is too long â€” trim to a single tight line.');
  if (!/[?!]/.test(firstLine)) issues.push('Hook lacks tension â€” try a question or bold claim.');
...
function mockRevision(userPrompt: string): string {
  ...
  if (!/talk with an advisor|build a more resilient|workforce planning|connected hr strategy|prepare your organization|read the full|link in bio|book a|â†’/i.test(joined)) {
...
function mockDirectorPlan(userPrompt: string): DirectorPlan {
  ...
  const rationales: Record<FormatId, string> = {
    ...
    email: 'Owned audience â€” captures intent from people already opted in.',
...
function mockCoherenceReport(userPrompt: string): CoherenceReport {
  ...
  if (ids.includes('linkedin') && ids.includes('twitter')) {
    notes.push('LinkedIn hook and Twitter opener share the same framing â€” good narrative consistency.');
  }
  ...
  if (ids.includes('instagram')) {
    notes.push('Instagram slides will need design pass â€” copy is ready, visuals are not.');
  }
  ...
  switch (id) {
    case 'linkedin':
      return [
        b.thesis,
        '',
        `Here is what we are seeing in workforce strategy today ðŸ‘‡`,
        '',
        ...k.map((p, i) => `${i + 1}. ${p}`),
        '',
        `The opportunity is not replacing expertise â€” it is scaling it with trusted guidance and practical solutions.`,
        '',
    ...
    case 'instagram': {
      const slides = [
        { t: 'Slide 1 â€” Hook', c: truncate(b.thesis, 120) },
        ...k.slice(0, 4).map((p, i) => ({ t: `Slide ${i + 2} â€” Key idea`, c: truncate(p, 140) })),
        { t: 'Slide 6 â€” CTA', c: 'Talk with an advisor about your next step â€” link in bio.' }
      ];
    ...
    case 'internal':
      return [
        ...
        '',
        `Key points:`,
        ...k.slice(0, 3).map((p) => `  â€¢ ${truncate(p, 160)}`),
        '',
